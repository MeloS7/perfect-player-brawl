...
const SIM_CONFIG = {
  ...
  BRAWL: {
    ...
    // ============================================================
    // 大乱斗专用：比赛胜负判定微调（只在 BRAWL.ENABLED 时生效）
    // 目标：拉开强弱、让关键值和你的发挥真正影响胜负
    // ============================================================
    SIM: {
      ENABLED: true,
      ...
      CLUTCH_MARGIN: 5,
      /** (2) 关键值每差 1 点、在最接近时最多挪的分数（分差 ≤ CLUTCH_MARGIN 才生效）；设 0 关闭 */
      CLUTCH_COEFF: 0.22,

      /** (3) 你单场发挥（相对自己近期状态 GmSc）对己方效率的影响：系数与最大分数摆动；设 0 关闭 */
      USER_IMPACT_COEFF: 0.30,
      ...
      /** 逐场累计全联盟每名球员的赛季数据（数据榜用真实值而非能力估算；额外开销 ~40ms/赛季） */
      LEAGUE_STATS: true,
    },
  },
